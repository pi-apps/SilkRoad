...
sales_data['competitor_price'] = np.random.uniform(100, 200, size=len(sales_data))
...
def optimize_pricing(product_id, base_price, competitor_price):
    # Define the input features
    X_new = pd.DataFrame({'product_id': [product_id], 'base_price': [base_price], 'competitor_price': [competitor_price]})

    # Predict the revenue
    revenue = xgb_model.predict(X_new)

    # Return the optimized price
    return base_price - 0.1 * (base_price - competitor_price) if revenue > 1000 else base_price
