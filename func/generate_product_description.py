...
def generate_product_description(product_name, num_sentences=3):
    ...
    input_ids = transformers.BertTokenizer.encode(product_name, return_tensors='pt')

    # Generate masked tokens for the product name
    masked_input_ids = input_ids.clone()
    masked_input_ids[0][0] = transformers.BertTokenizer.convert_tokens_to_ids('[MASK]')
    ...
    for i in range(num_sentences):
        ...
        for j in range(len(product_name)):
            token = transformers.BertTokenizer.convert_ids_to_tokens(masked_input_ids[0][j].item())
            sentence.append(transformers.BertTokenizer.decode(probs[0][j][:, i].argmax().item()))
    ...
    product_description = re.sub(r'\[MASK\]', random.choice(string.ascii_lowercase), product_description)
