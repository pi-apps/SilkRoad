...
def recommend_products(user_id, num_recommendations):
    ...
    recommendations = recommendation_model.recommend(
        user_purchases, num_recommendations
    )
...
def preprocess_user_purchase_data(user_purchase_data):
    ...
    user_purchase_matrix = sp.csr_matrix(
        (
            user_purchase_data["quantity"],
            (user_purchase_data["user_id"], user_purchase_data["product_id"]),
        )
    )
...
def evaluate_model(user_purchase_matrix, recommendation_model):
    ...
    evaluations = {}
    evaluations["precision"] = metrics.precision_score(
        user_purchase_matrix.todense(),
        recommendation_model.recommend(user_purchase_matrix, 10)[:, 0].astype(int),
    )
    evaluations["recall"] = metrics.recall_score(
        user_purchase_matrix.todense(),
        recommendation_model.recommend(user_purchase_matrix, 10)[:, 0].astype(int),
    )
    evaluations["f1_score"] = metrics.f1_score(
        user_purchase_matrix.todense(),
        recommendation_model.recommend(user_purchase_matrix, 10)[:, 0].astype(int),
    )
