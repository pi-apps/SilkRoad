...
def display_homepage(root):
    ...
    welcome_label = tk.Label(root, text="Welcome to our store!", font=tkFont.Font(family="Helvetica", size=24, weight="bold"))
    welcome_label.pack(pady=20)

    # Create a button for the product page
    product_button = tk.Button(root, text="View Products", font=tkFont.Font(family="Helvetica", size=18), command=lambda: display_product_page(root))
...
def display_product_page(root):
    ...
    product_title_label = tk.Label(root, text="Product Title", font=tkFont.Font(family="Helvetica", size=18, weight="bold"))
    product_title_label.pack(pady=10)

    # Create a label for the product description
    product_description_label = tk.Label(root, text="Product Description", font=tkFont.Font(family="Helvetica", size=14))
    ...
    add_to_cart_button = tk.Button(root, text="Add to Cart", font=tkFont.Font(family="Helvetica", size=14), command=lambda: handle_add_to_cart_button_click())
    add_to_cart_button.pack(pady=5)

    # Create a button for going back to the homepage
    back_button = tk.Button(root, text="Back", font=tkFont.Font(family="Helvetica", size=14), command=lambda: display_homepage(root))
