...
$(document).on("click", ".trans-button #xac-nhan-dat", function (e) {
    ...
    var what = new jPopup({
        title: "<h2>Thông báo piglobal.net</h2>",
        content: "<p>Bạn có chắc đã thực hiện thành công thao tác chuyển tiền?<br>Nếu bạn đã thực hiện thanh toán. Vui lòng nhấp vào liên kết bên dưới để tiếp tục.<br><div style='text-align:center;'><a data-order='" + $("#hdfCodeOrder").val() + "' href='javascript:void(0);' class='tien-hanh-bao-nguoi-ban btn btn-success'>Tiếp tục giao dịch</a></div></p>",
        draggable: true,
        resizeable: true,
        overlay: true,
        closeButton: true
...
$(document).on("click", ".tien-hanh-bao-nguoi-ban", function () {
    ...
    $("#overlay-mua-hang").show();
    $.ajax({
        url: '/transaction/SendTransactionForSellerBuyProduct.aspx',
        data: { order: order }, //data
        type: 'POST',
        success: function (data) {
            //console.log(data);
...
$(document).on("click", ".trans-button #xac-nhan-nhan-tien", function (e) {
    ...
    var what = new jPopup({
        title: "<h2>Thông báo piglobal.net</h2>",
        content: "<p>Bạn có chắc đã nhận được thanh toán từ người mua hàng?<br>Nếu bạn đã nhận được. Vui lòng nhấp vào liên kết bên dưới để tiếp tục.<br><div style='text-align:center;'><a data-order='" + $("#hdfCodeOrder").val() + "' href='javascript:void(0);' class='tien-hanh-hoan-thanh-don btn btn-success'>Tiếp tục xác nhận</a></div></p>",
        draggable: true,
        resizeable: true,
        overlay: true,
        closeButton: true
...
$(document).on("click", ".tien-hanh-hoan-thanh-don", function () {
    ...
    $("#overlay-mua-hang").show();
    $.ajax({
        url: '/transaction/FinishTransactionForBuyProduct.aspx',
        data: { order: order }, //data
        type: 'POST',
        success: function (data) {
            //console.log(data);
