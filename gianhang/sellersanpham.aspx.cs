...
public partial class gianhang_chitietsanpham : PrivateUser
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            hdfShop.Value = "/gian-hang/" + shopRental.Name.RemoveSymbol() + "-" + shopRental.Id + ".html";
            hdfUrl.Value = urlDomainShare + Request.RawUrl;
            hdfPurchaseFee.Value = sys_Config.PurchaseFee + "";
            hdfCodeOrder.Value = order.Id + "";
            if (order.IsBuyerConfirm == false && order.IsDeleted == false && order.IsSellerConfirm == false)
            {
                Panel2.Visible = true;
                ltrNguoiMua.Text = "Đang chờ người mua xác nhận chuyển tiền";
            }
            else if (order.IsBuyerConfirm == true && order.IsDeleted == false && order.IsSellerConfirm == false)
            {
                Panel2.Visible = true;
                ltrNguoiMua.Text = "Người mua đã xác nhận chuyển tiền";
            }

            if(order.IsBuyerConfirm == true && order.IsDeleted == false && order.IsSellerConfirm == false)
            {
                Panel1.Visible = true;
            }
