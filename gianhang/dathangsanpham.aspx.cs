...
public partial class gianhang_chitietsanpham : PrivateUser
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            if (order.IsDeleted == true || order.IsBuyerConfirm==true)
            {
                Panel1.Visible = false;
            }
            if (order.IsDeleted == false && order.IsBuyerConfirm == false)
            {
                Panel1.Visible = true;
            }
            if (order.IsDeleted ==false && order.IsBuyerConfirm == true)
            {
                Panel2.Visible = true;
            }
