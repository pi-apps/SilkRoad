...
public partial class gianhang_Default : PublicUser
{
    ...
    public pro_Category category { get; set; }
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        category = dbConnect.pro_Categories.Single(n => n.Id == int.Parse(Request["categorylId"]));
        ...
        var categoryResults = dbConnect.proGetChildLevelCategory(category.Id, category.Name).ToList();
        string ids = string.Join(",", categoryResults.Distinct().Select(n => n.child).Distinct());

        var items = dbConnect.proProductByCategory(ids,shopRental.Id, int.Parse(ddlSort.SelectedValue))
                       .Distinct().ToList();
